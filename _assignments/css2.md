...
3. Adjust the margins so that the top margin is 2em and the bottom margin is 0.5em.
...
2. We need to set a size for the Destination Kent State logo.  However, we only want to affect that image and not the other images on the page.  Add a class to the DKS logo within the HTML (you can choose any class name you like).
3.  Target that class in the CSS to be a width of 150px and have a top margin of 1em.
...
6. You have to set cite to display:block in order for it to have a margin and appear on its own line.
